	...
	return this.each(function () {
		...
		*/
		function drawLinks() {
			...
			//添加首页
			if (opts.first_text && (current_page > 0 || opts.first_show)) {
				appendItem(0, { text: opts.first_text, classes: "disabled" });
			}
			...
			//添加尾页
			if (opts.end_text && (current_page < np || opts.end_show_always)) {
				appendItem(np, { text: opts.end_text, classes: "disabled" });
			}
			if (opts.isJump) {
				// Generate Jump Input  
				panel.append(jQuery("<input type='text' id='jump-index' maxlength='4' style='width:20px;'/>"));
